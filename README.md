...
| Import                             | What it is                                                                                                                                                                                                                                                   |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `@boardwalk-labs/workflow`         | The author API: `agent()`, `sleep()`, `now()` / `random()` / `uuid()`, `step.run()`, `workflows.call()`, `secrets.get()`, `artifacts.write()`, `parallel()`, `input` / `output()` / `config`, `phase()` — plus the manifest schema and run-event wire format |
| `@boardwalk-labs/workflow/lint`    | Static determinism lint (AST-based): flags bare clock/random/network calls outside a journaled seam. Shared by the CLI (`boardwalk deploy` blocks on bare clock/random/uuid; `fetch` is advisory), engines, and hosted deploy                                |
...
- **`sleep(ms | { until })`** — durable wait; a short wait holds (locals survive), a long one suspends (the task is released and the run replays from the top on wake).
- **`now()` / `random()` / `uuid()`** — durable clock + randomness. Because a run replays from the top on resume/crash, a bare `Date.now()` / `Math.random()` / `crypto.randomUUID()` returns a _different_ value each segment and silently corrupts anything captured before a `sleep`. These capture the value once and memoize it (one `step` each). The `/lint` check flags the bare calls and `boardwalk deploy` blocks on them unless you pass `--allow-nondeterminism`. For arbitrary nondeterministic I/O, use **`step.run(name, fn)`** — run once, memoize the result.
